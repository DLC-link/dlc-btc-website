...
export function useEthereum(): UseEthereumReturn {
  ...
  async function fetchEthereumDeploymentPlan(contractName: string, chainID: string) {
    ...
    try {
      ...
      throw new Error(`Could not fetch deployment info for ${contractName} on ${network?.name}`);
    }
  }

  function getEthereumNetworkConfig(): { walletURL: string; explorerAPIURL: string } {
    switch (network?.id) {
  ...
  async function getVault(vaultUUID: string): Promise<void> {
    try {
      ...
      store.dispatch(vaultActions.swapVault({ vaultUUID, updatedVault: formattedVault }));
    } catch (error) {
